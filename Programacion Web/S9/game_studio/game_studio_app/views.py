...
def buscar_juegos(request):
    ...
    if request.method == 'POST':
        ...
        url = f"https://www.giantbomb.com/api/search/?api_key=6fb2f9287182fe65f3bca23dc7276faaf064a1a6&format=json&query='{nombre_juego}'&resources=game"
        response = requests.get(url, headers=headers)
        logger.debug(response.text)
        data = response.json()
        logger.debug(f'La informacion obtenida es: {response.text}')
...
def detalle_juego_seleccionado(request):
    if request.method == 'POST':
        ...
        url = f"https://www.giantbomb.com/api/game/{juego_id}/?api_key=6fb2f9287182fe65f3bca23dc7276faaf064a1a6&format=json"
        response = requests.get(url, headers=headers)
        data = response.json()
        juego = data['results']
